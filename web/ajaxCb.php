...
if($mySession->isLogged()) {
    ...
    TOGGLE SWITCH
    =========================================== */
    if($ajax_action == "cb_switch_toggle") {
	if(isset($_POST["id"])) {
	...
	// Request a new scan for all hosts in this network
	if(isset($_GET["id"])) {
	    $net_id = intval($_GET["id"]);
		...
		if(mysqli_num_rows($result) > 0) {
		    ...
		    $job->args = json_encode(array("net_addr" => stripslashes($row["Network"]),"scan_method" => ""));

		    if($job->schedule()) {
			echo "{\"message\":\"Net ".stripslashes($row["Network"])." rescan queued!\", \"type\":\"success\"}";
		    } else {
			echo "{\"message\":\"Scan already scheduled!\", \"type\":\"warning\"}";
		    }
		} else {
		    echo "{\"message\":\"Invalid params\", \"type\":\"error\"}";
		}
	    } else {
		echo "{\"message\":\"Forbidden\", \"type\":\"error\"}";
	    }
	} else {
	    echo "{\"message\":\"Invalid request\", \"type\":\"error\"}";
	}
    }
	...
	echo "{\"message\":\"Completed job cleaned!\", \"type\":\"success\"}";
    }
	...
	echo "{\"message\":\"Events log cleared!\", \"type\":\"success\"}";
    }
...
<?php
	...
	if(mysqli_num_rows($result) > 0) {
		...
		$ret_array["rows"][] = array("job" => $job_method, "id" => $job_id, "agent_id" => $job_agent_id, "args" => $job_args, "schedule_date" => $job_scheduledate->format("H:i:s d-M-Y"), "start_date" => ($job_startdate ? $job_startdate->format("H:i:s d-M-Y") : "Not yet"), "end_date" => ($job_enddate ? $job_enddate->format("H:i:s d-M-Y") : "Not yet"), "time_elapsed" => $job_timeelapsed);
	    }
	    header('Content-Type: application/json');
	    $json = json_encode($ret_array);
	    echo $json;
